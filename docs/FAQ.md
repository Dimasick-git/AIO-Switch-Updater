...
Это значение `APP_VERSION` из `Makefile`. Семантика чистая
upstream-семвер: major.minor.patch. Мы держим её совпадающей с
веткой upstream AIO-Switch-Updater, на которой собрана наша версия;
при наших изменениях патч-цифра увеличивается.

Эта строка показывается в About и используется кодом самообновления.
Git-тег релиза формируется как `v${APP_VERSION}` (например `v2.23.3`)
и публикуется на GitHub только при изменении `APP_VERSION` в Makefile.

## А есть ли pre-release / nightly?

Тег `nightly` — legacy. Промежуточные сборки не публикуются как
релизы — они доступны как Actions Artifact (вкладка Actions → нужный
run → "ryazhenka-updater-zip" / "ryazhenka-updater-nro"). Это
гарантирует, что страница Releases содержит только версионные
релизы, между которыми есть смысл сравнивать.
