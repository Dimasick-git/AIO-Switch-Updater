...
- Версия приложения синхронизирована во всех экранах, метаданных и релизных
  артефактах.
...
- Полный каталог профилей Joy-Con теперь встроен в приложение и отображается
  даже без доступа к GitHub. Ранее при недоступной сети показывался только
  резервный профиль Animal Crossing.

## [2.24.1] — 2026-09-18

### Added

- Каталог профилей цветов Joy-Con расширен до полного набора из upstream:
  добавлены стандартные, неоновые, тематические и специальные профили.

## [2.24.0] — 2026-08-19

### Fixed

- Исправлена вкладка **Atmosphère**: она снова читает актуальный ключ каталога
  `Atmosphère`, а latest-asset ссылки сначала разрешаются через GitHub API и
  только затем передаются загрузчику.
- Custom packs снова получают Hekate из раздела `bootloaders`; демонстрационные
  ссылки удалены, поэтому пустой каталог больше не показывает битые кнопки.
- Кнопка загрузки payload передаёт HTTP-статус рабочему экрану и не показывает
  ложное завершение при сетевой ошибке.
- Ручная проверка обновлений пака теперь сравнивает latest tag с тегом,
  записанным после успешной установки. Автопроверка показывает доступную версию,
  но не выполняет скрытую перезапись CFW.
- Исправлены сетевые настройки: русские подписи, корректный выход без создания
  второго главного экрана, стабильный LAN-play и правильный четвёртый октет IP.
- Восстановление `/atmosphere` передаёт ошибку в рабочий экран вместо ложного
  сообщения об успехе.
- Общий downloader корректно обрабатывает ошибку открытия временного файла и
  граничный размер буфера.

### Changed

- Список проектов экосистемы теперь честно открывает страницы проектов.
  Установка выполняется через каталог Ряженки, в котором для каждого NRO,
  overlay, sysmodule, payload и ZIP зафиксирован безопасный путь на SD.

## [2.23.9] — 2026-08-19

### Changed

- Каталог загрузок приведён к единому стилю: реальные названия проектов
  сохранены, а описания кнопок переведены на понятный русский язык.
- **Ryazhenka Tuner** теперь показан как настройки CFW; PPSSPP использует
  актуальную стандартную структуру `PPSSPP.zip`; Minecraft Shader Studio
  использует текущий репозиторий вместо устаревшего redirect.
- Hekate, Lockpick_RCM и TegraExplorer получили короткие названия кнопок,
  сохранив явные технические пути для payload-файлов.
- Проверка компонентов различает NRO, оверлеи, sysmodules и SD-пакеты:
  версия из NACP читается только для NRO, остальные компоненты корректно
  проверяются по наличию файла и latest-релизу.
- Удалены нерабочие источники проверки обновлений Status-Monitor-Overlay,
  nx-ovlloader и RCU до появления действующих release-каналов.
